...
export default function ProfileModal({ isOpen, onClose, user }) {
    ...
    // Details State (Initialize with prop, but update with fetch)
    ...
    // ✅ FIX: Fetch fresh profile data when modal opens
    useEffect(() => {
        if (isOpen) {
            setFetching(true);
                            ...
                            {activeTab === 'details' && (
                                <form onSubmit={handleUpdateDetails} className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 bg-slate-50 p-3 rounded-xl border border-slate-100">
                                        <div>
                                            <label className="text-xs font-bold text-slate-400 uppercase mb-1">Email</label>
                                            <input type="email" value={formData.email} disabled className="w-full border p-2 rounded-lg bg-white text-slate-500 cursor-not-allowed text-xs" />
                                        </div>
                                        <div>
                                            <label className="text-xs font-bold text-slate-400 uppercase mb-1">ID Number</label>
                                            <input type="text" value={formData.id_number} disabled className="w-full border p-2 rounded-lg bg-white text-slate-500 cursor-not-allowed text-xs" />
                                        </div>
                                    </div>
